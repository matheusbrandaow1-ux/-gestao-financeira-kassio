...
export const ConceptAssemblyView: React.FC = () => {
  ...
  const patrimonial = useMemo(() => getConsolidatedNetWorth(assets, currency), [assets, currency]);
  ...
  const budget = useMemo(() => {
    ...
    monthTransactions.filter(transaction => transaction.transactionType === 'DESPESA').forEach(transaction => {
      ...
      result.set(key, (result.get(key) || 0) + (getCanonicalBaseAmount(transaction) ?? 0));
    });
    const summary = getFinancialSummary(monthTransactions);
    return { income: summary.income, investments: summary.investments, values: result };
  }, [categories, monthTransactions]);

  const liquidity = useMemo(() => {
    let total = 0;
    for (const account of accounts.filter(account => ['CHECKING', 'SAVINGS', 'CASH'].includes(account.type))) {
  ...
  const reserveMonths = reserveExpenses > 0 && liquidity !== undefined ? liquidity / reserveExpenses : undefined;
  ...
  return (
    <div className="ap-view space-y-8 pb-12">
      ...
      <Notice>Montagem pontual: esta leitura é gerada sob demanda a partir dos dados atuais e não cria cron, listener ou execução automática.</Notice>
      ...
      <section className="border-b border-slate-800/80 pb-7">
        ...
        <div className="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-3">
          <div><p className="text-xs text-slate-500">Patrimônio líquido comparável</p><p className="mt-1 font-mono text-xl text-slate-100">{!patrimonial.isComplete ? 'Conversão indisponível' : formatCurrency(patrimonial.netWorth, currency)}</p></div>
          <div><p className="text-xs text-slate-500">Liquidez</p><p className="mt-1 font-mono text-xl text-slate-100">{liquidity === undefined ? 'Conversão indisponível' : formatCurrency(liquidity, currency)}</p></div>
      ...
      <section className="space-y-5">
        ...
        <div className="grid grid-cols-1 gap-5 lg:grid-cols-2">
          <DataTable rows={[
            ['Ativos', patrimonial.isComplete ? formatCurrency(patrimonial.assets, currency) : 'Conversão indisponível'],
            ['Passivos', patrimonial.isComplete ? formatCurrency(patrimonial.liabilities, currency) : 'Conversão indisponível'],
            ['Investimentos', `${investmentAssets.length} posições únicas`],
            ['Liquidez', liquidity === undefined ? 'Conversão indisponível' : formatCurrency(liquidity, currency)]
          ]} />
          <DataTable rows={currencyTotals.map(item => [item.code, formatCurrency(item.value, item.code)])} />
        </div>
        {!patrimonial.isComplete && <Notice>Conversão CHF indisponível; valores originais permanecem preservados.</Notice>}
      </section>
