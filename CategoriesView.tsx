...
import { getCanonicalBaseAmount } from '../lib/canonicalFinance';
...
export const CategoriesView: React.FC = () => {
  ...
  const categoryCoverage = useMemo(() => {
    ...
    const categorizedValue = expenses
      ...
      .reduce((sum, transaction) => sum + Math.abs(getCanonicalBaseAmount(transaction) ?? 0), 0);
    const uncategorizedValue = uncategorized.reduce((sum, transaction) => sum + Math.abs(getCanonicalBaseAmount(transaction) ?? 0), 0);
