...
router.post('/sync', requireAuth, async (req: Request, res: Response) => {
  const { clientId, startDate, updatedSince } = req.body || {};
  ...
  try {
    // The server is the source of truth. Never trust browser-provided snapshots for reconciliation.
    const [existingTransactions, existingAccounts, existingCategories, existingRules, existingRecurring, persistedCorrections] = await Promise.all([
      FirestoreRepository.list(authorizedClientId, 'transactions'),
      FirestoreRepository.list(authorizedClientId, 'accounts'),
      FirestoreRepository.list(authorizedClientId, 'categories'),
      FirestoreRepository.list(authorizedClientId, 'rules'),
      FirestoreRepository.list(authorizedClientId, 'recurringItems'),
      FirestoreRepository.list(authorizedClientId, 'aiCorrections')
    ]);
    humanCorrectionStore.hydrateClientCorrections(authorizedClientId, persistedCorrections as any);
    ...
    const result = await syncService.runSync(authorizedClientId, {
      startDate,
      updatedSince,
      existingTransactions: existingTransactions as any,
      existingAccounts: existingAccounts as any,
      existingCategories: existingCategories as any,
      existingRules: existingRules as any,
      existingRecurring: existingRecurring as any
    });

    // Persist the sync atomically from the server's authorization context. This also
    // allows CLIENT users to refresh their own data without granting browser Firestore writes.
    const persistenceWrites: Promise<void>[] = [];
    for (const account of result.accountsToPersist) {
      persistenceWrites.push(FirestoreRepository.upsert(authorizedClientId, 'accounts', account.id, account as any));
    }
    for (const category of result.categoriesToPersist) {
      persistenceWrites.push(FirestoreRepository.upsert(authorizedClientId, 'categories', category.id, category as any));
    }
    const existingTransactionById = new Map((existingTransactions as any[]).map(transaction => [String(transaction.id), transaction]));
    const closedMonths = new Map<string, boolean>();
    const monthIsClosed = async (month: string): Promise<boolean> => {
      if (!closedMonths.has(month)) closedMonths.set(month, await isMonthlyCloseClosed(authorizedClientId, month));
      return closedMonths.get(month)!;
    };
    const safeTransactionsToPersist = [] as any[];
    for (const transaction of result.transactionsToPersist) {
      const month = String(transaction.date || '').slice(0, 7);
      if (month && await monthIsClosed(month)) continue;
      safeTransactionsToPersist.push(transaction);
      persistenceWrites.push(FirestoreRepository.upsert(authorizedClientId, 'transactions', transaction.id, transaction as any));
    }
    // A closed monthly snapshot must stay immutable even if the provider changes later.
    const safeTransactions = [] as any[];
    for (const transaction of result.transactions) {
      const month = String(transaction.date || '').slice(0, 7);
      if (month && await monthIsClosed(month)) {
        const previous = existingTransactionById.get(String(transaction.id));
        if (previous) safeTransactions.push(previous);
        continue;
      }
      safeTransactions.push(transaction);
    }
    result.transactionsToPersist = safeTransactionsToPersist;
    result.transactions = safeTransactions;

    for (const recurring of result.recurringItems) {
      persistenceWrites.push(FirestoreRepository.upsert(authorizedClientId, 'recurringItems', recurring.id, recurring as any));
    }
    persistenceWrites.push(FirestoreRepository.upsert(authorizedClientId, 'syncJobs', result.job.id, result.job as any));
