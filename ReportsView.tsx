...
import { getCanonicalBaseAmount, getFinancialSummary } from '../lib/canonicalFinance';
...
export const ReportsView: React.FC = () => {
  ...
  // Aggregate Metrics
  const filteredSummary = useMemo(() => getFinancialSummary(filteredTransactions), [filteredTransactions]);
  const totalIncome = filteredSummary.income;
  const totalExpenses = filteredSummary.expenses;
  const totalInvestments = filteredSummary.investments;
  ...
  const categoryExpenses = useMemo(() => {
    ...
    filteredTransactions
      ...
      .forEach(t => {
        ...
        map[catName].amount += getCanonicalBaseAmount(t) ?? 0;
      });
