...
5. Sempre formate valores monetários com a moeda ${financialContext.baseCurrency} (ex: CHF 1'250.00).`;
  ...
  public async generateMonthlyInsights(params: MonthlyInsightsParams): Promise<MonthlyInsightsResult> {
    ...
    const refundTransactions = transactions.filter(t => t.type === 'ESTORNO');
    ...
    const grossExpenses = expenseTransactions.reduce((acc, t) => acc + Math.abs(t.amount || 0), 0);
    const refunds = refundTransactions.reduce((acc, t) => acc + Math.abs(t.amount || 0), 0);
...
- Não invente nada.`;
  ...
  public async generateMonthlySummary(params: MonthlySummaryParams): Promise<MonthlyFinancialSummaryReport> {
    ...
    const incomeRealized = transactions.filter(t => t.type === 'RECEITA').reduce((s, t) => s + Math.abs(t.amount || 0), 0);
    const grossExpensesRealized = transactions.filter(t => t.type === 'DESPESA').reduce((s, t) => s + Math.abs(t.amount || 0), 0);
    const refundsRealized = transactions.filter(t => t.type === 'ESTORNO').reduce((s, t) => s + Math.abs(t.amount || 0), 0);
    const expensesRealized = grossExpensesRealized - refundsRealized;
    const investmentsRealized = transactions.filter(t => t.type === 'INVESTIMENTO').reduce((s, t) => s + Math.abs(t.amount || 0), 0);
    // Canonical operating result: investments are tracked separately, not treated as expenses.
    const netResultRealized = incomeRealized - expensesRealized;
    ...
    for (const t of transactions.filter(tx => tx.type === 'DESPESA' || tx.type === 'ESTORNO')) {
      const catName = t.category || 'Outros';
      const amount = Math.abs(t.amount || 0) * (t.type === 'ESTORNO' ? -1 : 1);
