...
class HumanCorrectionStore {
  ...
  public hydrateClientCorrections(clientId: string, records: HumanCorrectionRecord[]): void {
    for (const record of records) {
      if (!record || record.clientId !== clientId) continue;
      if (record.merchant) {
        this.corrections.set(`${clientId}:::M:::${this.normalizeKey(record.merchant)}`, record);
      }
      if (record.originalDescription) {
        this.corrections.set(`${clientId}:::D:::${this.normalizeKey(record.originalDescription)}`, record);
      }
      if (!this.history.some(item => item.id === record.id)) this.history.push(record);
    }
