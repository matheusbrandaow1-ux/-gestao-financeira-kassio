...
export const ClientProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const recategorizeTransaction = async (id: string, categoryId: string, subcategoryName?: string) => {
    ...
    if (!response.ok || !body.success || !body.updatedTransaction) {
      throw new Error(body.message || 'Não foi possível salvar a categorização.');
    }

    const updated = body.updatedTransaction as CanonicalTransaction;
    setTransactions(prev => prev.map(t => t.id === id ? updated : t));
    void addAuditLog(
      'RECATEGORIZAR_TRANSACAO',
      'TRANSACTIONS',
      `Transação ${id} categorizada como ${updated.categoryName || categoryId}`,
      'USER'
    ).catch(() => undefined);
  };
  ...
  const triggerLunchMoneySync = async (): Promise<{ success: boolean; message: string }> => {
    ...
    try {
      const response = await fetch('/api/lunchmoney/sync', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ clientId: currentTargetClientId })
      });
      ...
      if (resData.success && resData.data) {
        ...
        // Persistence is performed server-side by /api/lunchmoney/sync.
        // The browser only refreshes its local snapshot after a successful sync.

        if (Array.isArray(syncData.accounts)) setAccounts(syncData.accounts);
        if (Array.isArray(syncData.categories)) setCategories(syncData.categories);
        if (Array.isArray(syncData.transactions)) setTransactions(syncData.transactions);
        if (Array.isArray(syncData.recurringItems)) setRecurringItems(syncData.recurringItems);
        if (job) {
