...
import { getFinancialSummary, getCategoryRealizedMap } from '../lib/canonicalFinance';
...
export const PlanningView: React.FC = () => {
  ...
  const realizedSummary = useMemo(() => getFinancialSummary(monthTransactions), [monthTransactions]);
  const realizedIncome = realizedSummary.income;
  const realizedExpenses = realizedSummary.expenses;
  const realizedInvestments = realizedSummary.investments;
  ...
  // Category execution is derived from the same canonical engine used by the KPIs.
  const categoryRealizedMap = useMemo(() => getCategoryRealizedMap(monthTransactions), [monthTransactions]);
  ...
  const handleSavePlan = async () => {
    ...
    const updated: MonthlyPlan = {
      ...
      id: selectedMonth,
      month: selectedMonth,
      plannedIncome: sumIncome,
      plannedExpenses: sumExp,
      plannedInvestments: sumInv,
