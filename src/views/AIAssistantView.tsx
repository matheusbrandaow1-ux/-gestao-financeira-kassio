...
export const AIAssistantView: React.FC = () => {
  ...
  const buildFinancialContext = () => {
    ...
    return {
      ...
      currentMonthTransactions: transactions.slice(0, 40).map(t => ({
        date: t.date,
      ...
      realizedTotals: {
        income: incomeRealized,
        expenses: expensesRealized,
        investments: investmentsRealized,
        netResult
      },
    ...
    try {
      const res = await fetch('/api/ai/monthly-summary', {
        ...
        body: JSON.stringify({
          ...
          monthlyPlan: {
            plannedIncome: monthlyPlan.plannedIncome,
            plannedExpenses: monthlyPlan.plannedExpenses,
            plannedInvestments: monthlyPlan.plannedInvestments
          },
          goals: goals.map(g => ({
            name: g.name,
