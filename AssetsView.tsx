...
import { getAssetBaseValue, getConsolidatedNetWorth } from '../lib/canonicalFinance';
...
export const AssetsView: React.FC = () => {
  ...
  const netWorthSummary = getConsolidatedNetWorth(assets, currency);
  const totalAssets = netWorthSummary.assets;
  const totalLiabilities = netWorthSummary.liabilities;
  const netWorth = netWorthSummary.netWorth;

  // Allocation uses the same consolidated base values as the headline. Missing FX is
  // excluded from the chart and explicitly signaled instead of being represented as zero.
  const assetAllocationData = assets
    .filter(a => a.classification === 'ATIVO')
    .map(a => ({ name: a.name, value: getAssetBaseValue(a, currency) }))
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      {!netWorthSummary.isComplete && (
        <div className="rounded-xl border border-amber-500/30 bg-amber-950/20 px-4 py-3 text-xs text-amber-200">
          {netWorthSummary.unavailableConversionCount} item(ns) preservado(s) na moeda original com conversão {currency} indisponível. Eles não foram tratados como zero nem somados incorretamente ao consolidado.
        </div>
      )}
