...
export class LayeredCategorizationEngine {
  ...
  public async classifyTransaction(
    ...
    // =========================================================================
    // LEVEL 5 — AI_ONLY_WHEN_UNKNOWN (provider AI + optional web research)
    // =========================================================================
    ...
    return await aiQuotaManager.executeWithQuotaControl(
      async () => {
        ...
        if (config.enableSearchGrounding && aiProvider.name !== 'OPENAI' && defaultMerchantResearchProvider.isAvailable()) {
          try {
        ...
        // 5b. Provider semantic reasoning. OpenAI can use its server-side web-search tool
        // for unknown public merchants; Gemini keeps its dedicated grounding step above.
        if (aiProvider.isAvailable()) {
          const aiResult = await aiProvider.classifyMerchant({
            transaction,
            availableCategories,
            useSearchGrounding: aiProvider.name === 'OPENAI' 
          });
